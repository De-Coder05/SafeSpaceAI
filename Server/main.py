...
def process_csv_data(csv_buffer):
    ...
    try:
        ...
        if len(data) == 0:
            raise ValueError("CSV file is empty. Please provide a CSV file with data.")
        
        print(f"📊 CSV file loaded: {len(data)} rows, {len(data.columns)} columns")
        print(f"📋 Available columns: {list(data.columns)}")
        
        # Validate required columns
        required_sensors = CFG["sensors"]
        available_columns = list(data.columns)
        missing_sensors = [sensor for sensor in required_sensors if sensor not in data.columns]
        
        if missing_sensors:
            print(f"⚠️  Warning: Missing sensors: {missing_sensors}")
            print(f"📋 Available columns: {available_columns}")
            print(f"🔍 Required sensors: {required_sensors}")
            print("💡 Note: Missing sensors will be filled with zeros")
        
        window_size = STEP
        stride_size = STRIDE
        
        # Check if data has enough rows for at least one window
        min_required_rows = window_size
        if len(data) < min_required_rows:
            raise ValueError(
                f"Insufficient data: CSV file has {len(data)} rows, but needs at least {min_required_rows} rows "
                f"(window size: {CFG['window_sec']} seconds × {CFG['fs']} Hz = {window_size} samples). "
                f"Please provide a longer data file."
            )
        
        print(f"✅ Data length check passed: {len(data)} rows >= {min_required_rows} rows (minimum)")
        
        all_features = []
        
        # Calculate number of windows that can be extracted
        num_windows = len(range(0, len(data) - window_size + 1, stride_size))
        print(f"📊 Processing {num_windows} window(s) from {len(data)} rows...")
        ...
        if not all_features:
            raise ValueError(
                f"No features extracted from data. "
                f"CSV file has {len(data)} rows, but needs at least {min_required_rows} rows. "
                f"Required columns: {required_sensors}. "
                f"Available columns: {available_columns}"
            )
        
        print(f"✅ Successfully extracted {len(all_features)} feature window(s)")
        ...
        print(f"   CSV shape: {data.shape if 'data' in locals() else 'N/A'}")
        print(f"   Available columns: {list(data.columns) if 'data' in locals() else 'N/A'}")
